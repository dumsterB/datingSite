<template>
  <div class="profile profile-page profile-swipe">
    <div class="content"  v-if="user.profile">
      ...
      <div class="row">
        <Photos :photos="user && user.profile.pictures"  :per-page-custom="[[2560,6],[1920,4],[1500, 4], [1440,4], [1250,3] ]"/>
      </div>
      <div class="row">
        <div class="profile-page__info profile-swipe__info">
         <div class="left-block">
           <p class="name online">{{ user.profile.name }}, <span class="age"> {{ user.profile.birth_date | birthday }}</span></p>
           <span class="city">Odessa <span>32km</span> </span>
           <p class="profile-swipe__about">Lorem Lorem labore exercitation mollit magna qui amet aute elit id occaecat culpa velit...</p>
...
export default {
  ...
  methods:{
    ...
    chatEnter(){
      ...
      console.log(this.$store.state.chat.chatRoute)
      this.$router.push({path: `/chat/${this.$store.state.chat.chatRoute.params.id}`, query: {chat_id: this.$store.state.chat.chatRoute.query.id}});
